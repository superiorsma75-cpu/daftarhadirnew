...
<body>
    ...
    <script>
        ...
        function downloadTeacherCSV() {
            // Get all data or filtered data based on class
            let dataToExport = state.attendance;
            
            // If we are in Monthly tab, maybe only export that month?
            // Usually for "Send to Student/Teacher", they want full history.
            // But let's sort it by Date descending first.
            ...
            dataToExport.forEach(row => {
                ...
                const dateStr = d.toLocaleDateString('id-ID'); // DD/MM/YYYY
                const timeStr = d.toLocaleTimeString('id-ID', {hour12: false}); // HH:MM:SS
                
                // Find class for student (because class isn't in attendance object directly, only studentId)
